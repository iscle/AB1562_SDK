...
// static const uint16_t g_spi_master_pri_code[HAL_SPI_MASTER_MAX] = {SPI_MST0_IRQ_PRIORITY};
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_register_suspend_callback(SLEEP_BACKUP_RESTORE_SPI_MST0, (sleep_management_suspend_callback_t)spi_master_backup_register_callback, NULL);
    sleep_management_register_resume_callback(SLEEP_BACKUP_RESTORE_SPI_MST0, (sleep_management_resume_callback_t)spi_master_restore_register_callback, NULL);
#endif
...
#ifdef HAL_DVFS_MODULE_ENABLED
    dvfs_lock_control("SPI_M",DVFS_CURRENT_SPEED, DVFS_LOCK);
#endif

    spi_master_start_transfer_fifo(master_port, true);

#ifdef HAL_DVFS_MODULE_ENABLED
    dvfs_lock_control("SPI_M",DVFS_CURRENT_SPEED, DVFS_UNLOCK);
#endif
