...
*/
ATTR_TEXT_IN_TCM void SFI_MacLeave(void)
{
    uint32_t val;
...
*/
ATTR_TEXT_IN_RAM void SFI_GPRAM_Write(uint32_t gpram_offset, void *buff, uint32_t length)
{
    uint32_t i;
...
uint8_t  NOR_FLASH_LeaveDPDCmd;
uint32_t NOR_FLASH_FATStartAddr;
uint32_t NOR_FLASH_FATStartValue;
uint32_t SFI_MAC_CTL_BACKUP;
uint32_t SFI_DIRECT_CTL_BACKUP;
uint32_t SFI_MISC_CTL_BACKUP;
uint32_t SFI_MISC_CTL2_BACKUP;
uint32_t SFI_DLY_CTL_3_BACKUP;
...
uint32_t SFI_CLK_SWITCH_MISC_CTL_BACKUP;
uint32_t SFI_CLK_SWITCH_DLY_CTL_3_BACKUP;
