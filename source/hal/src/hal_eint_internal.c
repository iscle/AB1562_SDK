...
log_create_module(eint, PRINT_LEVEL_INFO);

#define EINT_LOGI(fmt,arg...)           LOG_I(eint,fmt,##arg)
#define EINT_LOGW(fmt,arg...)           LOG_W(eint,fmt,##arg)
#define EINT_LOGE(fmt,arg...)           LOG_E(eint,fmt,##arg)
#define EINT_MSGIDI(fmt,cnt,arg...)     LOG_MSGID_I(eint,fmt,cnt,##arg)
#define EINT_MSGIDW(fmt,cnt,arg...)     LOG_MSGID_W(eint,fmt,cnt,##arg)
#define EINT_MSGIDE(fmt,cnt,arg...)     LOG_MSGID_E(eint,fmt,cnt,##arg)
    ...
    for (eint_index = 0; eint_index < HAL_EINT_NUMBER_MAX; eint_index++) {
        if (status & (1 << eint_index)) {
            ...
            if (eint_function_table[eint_index].eint_callback) {
                ...
                EINT_LOGW("ERROR: no EINT interrupt handler!\n");
            }
        }
    }
}
