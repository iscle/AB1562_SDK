 ...
 * Public Functions
 ****************************************************************************/
void syslog_debug_uart_init(void)
{
    ...
    if(debug_switch == true){
        ...
        hal_uart_init(SYSLOG_DEBUG_UART, &uart_config);
...
int debug_printf(const char *message, ...)
{
    ...
    if(is_io_ready == false || debug_switch ==false){
        return 0;
    }
    ...
    va_end(args);
    hal_uart_send_polling(SYSLOG_DEBUG_UART, (uint8_t *)frame_header, log_size);  

    return log_size;
}

int debug_dump(const uint8_t *data, uint32_t size)
{
    if(is_io_ready == false || debug_switch ==false){
        return 0;
    }

    hal_uart_send_polling(SYSLOG_DEBUG_UART, (uint8_t *)data, size);  
