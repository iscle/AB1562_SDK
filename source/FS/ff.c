...
)
{
	...
	for (;;) {
		...
		if (di >= _MAX_LFN)
			return FR_INVALID_NAME + 20;	/* Reject too long name */
#if !_LFN_UNICODE
		w &= 0xFF;
		if (IsDBCS1(w)) {				/* Check if it is a DBC 1st byte (always false on SBCS cfg) */
			...
			if (!IsDBCS2(b))
				return FR_INVALID_NAME + 21;	/* Reject invalid sequence */
		}
		w = ff_convert(w, 1);			/* Convert ANSI/OEM to Unicode */
		if (!w) return FR_INVALID_NAME + 22;	/* Reject invalid code */
#endif
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	for (;;) {
		...
		if (IsDBCS1(c)) {				/* Check if it is a DBC 1st byte (always false at SBCS cfg.) */
			...
			if (!IsDBCS2(d) || i >= ni - 1)
				return FR_INVALID_NAME + 24;	/* Reject invalid DBC */
			sfn[i++] = c;
			sfn[i++] = d;
		} else {						/* SBC */
			if (chk_chr("\"*+,:;<=>\?[]|\x7F", c))
				return FR_INVALID_NAME + 25;	/* Reject illegal chrs for SFN */
...
#else
	if ((UINT)*path == 0x00) {				/* Null path name is the origin directory itself */
#endif
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if (fmt == 1) {
		...
		if (maxlba >= 0xFFFFFFFF) return FR_NO_FILESYSTEM;	/* (It cannot be handled in 32-bit LBA) */
