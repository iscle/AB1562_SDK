...
extern HandlerData gAppPeqTimerHandle;
...
void APP_PeqManage_ChangePeqGroup(void)
{
    ...
    switch(APP_AudioDspGetMainPath())
    {
        ...
        case AUDIO_LINEIN:
            ...
            break;
        #endif
		#if MP3_LOCAL_PLAYBACK_MMI_CTRL
		case AUDIO_MP3:
			peq_path = PEQ_MP3;
			break;
...
void APP_PeqManage_RealtimeUpdate(U8 *pPeqPara, U16 length)
{
	...
	if(FW_CheckTimer(&gAppPeqTimerHandle, AWSMCE_PEQ_SYNC_PLAYING_TIMER, 0))
	{
		DBG_LOG_APP_PEQ( "[APP_PEQ]realtime timer is active, not to do PEQ change", 0);
		return;
	}

	#ifdef APP_PEQ_SYNC_APPLY_TICK
	APP_Peq_SetAgentParameter(pPeqPara, length);
	APP_Peq_SetSyncTimer(BtAwsMce_GetCurrentBtClock());
	#else

	APP_PeqProc_RealtimeUpdate(*(pPeqPara), (pPeqPara + 1), length - 1);

	if(pBdAddr && BtAwsMce_IsDefaultRoleAgent()&&
		APP_AudioDspIsStreaming(pBdAddr, AUDIO_A2DP)) // agent role
	{
		APP_MCSYNC_SendPEQRealTimeInfo(pPeqPara, length);
	}
