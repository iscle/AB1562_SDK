...
{
	U8 a2dpLevel = System_ReadNvkeyA2dpSoundLevelFactoryValue();
	U8 scoLevel = System_ReadNvkeyScoSoundLevelFactoryValue();
	U8 micScoLevel = System_ReadNvkeyMicScoSoundLevelFactoryValue();
	U8 vpRtLevel = System_ReadNvkeyVpRtSoundLevelFactoryValue();
	
	 DBG_LOG_APP_SYSTEM( "[APP_Volume] soundlevel factory valuy:%d %d %d %d", 4, 
	 	a2dpLevel, scoLevel, micScoLevel, vpRtLevel);

	if(pA2dpSetting)
	{
		U16 a2dpDataSize = NVKEY_GetPayloadLength(NVKEYID_APP_VOLUME_A2DP_SETTING);

		if(a2dpDataSize <= 0)
			FW_Assert(FALSE);
		
		VOLUME_A2DP_SETTING_STRU *pA2dpData = (VOLUME_A2DP_SETTING_STRU*)FW_GetMemory(a2dpDataSize);

		if(pA2dpData)
		{
			FW_Memcpy(pA2dpData, pA2dpSetting , a2dpDataSize);
			pA2dpData->a2dpVolPara.defaultSoundLevel = a2dpLevel;
			nvkey_hdl_save_nvkey(NVKEYID_APP_VOLUME_A2DP_SETTING, a2dpDataSize, pA2dpData);
			FW_FreeMemory(pA2dpData);
		}
	...
	if(pScoSetting)
	{
		U16 scoDataSize = NVKEY_GetPayloadLength(NVKEYID_APP_VOLUME_SCO_SETTING);

		if(scoDataSize <= 0)
			FW_Assert(FALSE);
		
		VOLUME_SCO_SETTING_STRU *pScoData = (VOLUME_SCO_SETTING_STRU*)FW_GetMemory(scoDataSize);

		if(pScoData)
		{
			FW_Memcpy(pScoData, pScoSetting , scoDataSize);
			pScoData->scoVolPara.defaultSoundLevel = scoLevel;
			nvkey_hdl_save_nvkey(NVKEYID_APP_VOLUME_SCO_SETTING, scoDataSize, pScoData);
			FW_FreeMemory(pScoData);
		}
	}
	
	if(pScoMicSetting)
	{
		U16 scoMicDataSize = NVKEY_GetPayloadLength(NVKEYID_APP_VOLUME_SCOMIC_SETTING);

		if(scoMicDataSize <= 0)
			FW_Assert(FALSE);
		
		VOLUME_SCO_MIC_SETTING_STRU *pScoMicData = (VOLUME_SCO_MIC_SETTING_STRU*)FW_GetMemory(scoMicDataSize);

		if(pScoMicData)
		{
			FW_Memcpy(pScoMicData, pScoMicSetting , scoMicDataSize);
			pScoMicData->scoMicVolPara.defaultSoundLevel = micScoLevel;
			nvkey_hdl_save_nvkey(NVKEYID_APP_VOLUME_SCOMIC_SETTING, scoMicDataSize, pScoMicData);
			FW_FreeMemory(pScoMicData);
		}
	}

	if(pVpRtSetting)
	{
		U16 vpRtDataSize = NVKEY_GetPayloadLength(NVKEYID_APP_VOLUME_VPRT_SETTING);

		if(vpRtDataSize <= 0)
			FW_Assert(FALSE);
		
		VOLUME_VPRT_SETTING_STRU *pVpRtData = (VOLUME_VPRT_SETTING_STRU*)FW_GetMemory(vpRtDataSize);

		if(pVpRtData)
		{
			FW_Memcpy(pVpRtData, pVpRtSetting , vpRtDataSize);
			pVpRtData->vprtVolPara.defaultSoundLevel = vpRtLevel;
			nvkey_hdl_save_nvkey(NVKEYID_APP_VOLUME_VPRT_SETTING, vpRtDataSize, pVpRtData);
			FW_FreeMemory(pVpRtData);
		}
	}
}
