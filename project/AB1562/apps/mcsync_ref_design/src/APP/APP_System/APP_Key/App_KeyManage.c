...
static U32 app_KeyManage_Handler(Handler handler, U16 id, void *payload, U32 id_ext)
{
    ...
    if(id_ext == KEYALL && id == EVENT_KEY_UP)
    {
        App_SniffEnable(APP_GetServiceBdAddr(), APP_SNIFF_KEY);
    }
    else if(id == EVENT_KEY_DOWN)
    {
        App_SniffDisable(APP_GetServiceBdAddr(), APP_SNIFF_KEY);
    }

    if(id_ext < TOTAL_KEY_NUM /*&& !APP_NO_COMBO_KEY_MODE_FEAT */)
    {
        if(id == EVENT_KEY_DOWN)
        {
        	APP_Customer_CaptouchPressHandler(id_ext);
            APP_Customer_SendOtherKeyStatus(id_ext, Customer_KeyPress);
        }
        else if(id == EVENT_KEY_UP)
        {
        	APP_Customer_CaptouchReleaseHandler(id_ext);
            APP_Customer_SendOtherKeyStatus(id_ext, Customer_KeyRelease);
        }
    }
