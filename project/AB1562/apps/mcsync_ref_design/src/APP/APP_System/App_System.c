...
=======
			...
			DBG_LOG_APP_SYSTEM( "[APP_SYS]acl take over:%d %d %d", 3,
				APP_Conn_CheckTimer(TIMER_ID_CHECK_TAKE_OVER_RECONNECT)	,
				BtAwsMce_IsSpecialLink((BD_ADDR_T *)&ind->bdaddr), BtAwsMce_IsDefaultRoleAgent());

			if(APP_Conn_CheckTimer(TIMER_ID_CHECK_TAKE_OVER_RECONNECT) &&
				!BtAwsMce_IsSpecialLink((BD_ADDR_T *)&ind->bdaddr) && BtAwsMce_IsDefaultRoleAgent())
			{
				APP_Conn_ReleaseTimer(TIMER_ID_CHECK_TAKE_OVER_RECONNECT);
			}
			...
			{
               	 	if(!FW_CmpBdAddr((BD_ADDR_T *)&ind->bdaddr, BtAwsMce_ReadLocalBdAddr()))
					{
                    	APP_Mcsync_ClearNotReconnMask(MCS_RECONN_MASK_AG_DISC);
                	}
					...
					if(!BtAwsMce_IsSpecialLink((BD_ADDR_T *)&ind->bdaddr))
					{
                    	//always start timer to check if HFP(HSP)/A2DP/AVRCP is activated.
                   		 PM_ConnectionConflictStateMachine((BD_ADDR_T *)&ind->bdaddr, PM_CONN_CONFLICT_ACL_OPEN_EVT);
                    ...
                    APP_Conn_ReleaseTimer(TIMER_ID_POWER_OFF_AWS_CHECK); // OPPOX20E1-231
					#endif
					}
            }
...
=======
		case HCI_ERCODE_CONNECTION_TERMINATED_BY_LOCAL_HOST: //instead of APP_IsLinkActiveDisconnecting()
			...
			{
				if(!APP_ReConn_QueueTypeNum(RECONNECT_PWR_ON))
					APP_Mcsync_SetNotReconnMask(MCS_RECONN_MASK_AG_DISC); //partner may not reconnect agent immediately, preventing agent reconnect SP during super reconnect mode
			}
			else //not APP active disconnect, may be GAP (because there is no profile)
			{
...
=======
		...
		if(!APP_GetAGNum() && BtAwsMce_IsDefaultRoleAgent() && APP_Conn_CheckTimer(TIMER_ID_ACL_LINK_TIMER))
		{
			switch(state)
			{
				case MCSYNC_TAKE_OVER_CONNECTION_DISCONNECTED_LOW_PRIO_COMPLETE_STATE_AND_CONNECT_HIGH_PRIO_AWS:
					APP_Conn_SetTimer(TIMER_ID_CHECK_TAKE_OVER_RECONNECT, 2500);
					//APP_ReConn_Reconnect(RECONNECT_USER_INIT);
					//BtMCSync_SetMcsyncTakeOverConnState(MCSYNC_TAKE_OVER_CONNECTION_IDLE_STATE);

				case MCSYNC_TAKE_OVER_CONNECTION_IDLE_STATE:
					APP_Conn_ReleaseTimer(TIMER_ID_ACL_LINK_TIMER);
					break;

				case MCSYNC_TAKE_OVER_CONNECTION_TAKE_OVER_COMPLETE_STATE:
					APP_Conn_ReleaseTimer(TIMER_ID_TAKE_OVER_SLC_CONNECTED);
					APP_Conn_ReleaseTimer(TIMER_ID_ACL_LINK_TIMER);
					BtMCSync_SetMcsyncTakeOverConnState(MCSYNC_TAKE_OVER_CONNECTION_IDLE_STATE);
...
static BOOL app_System_CheckIsAllowToTakeOverConnection(BD_ADDR_T *pBdAddr)
{
	...
	BOOL isSCONotAllowTakeOverState = FALSE;
	UNUSED(pBdAddr);

	if(pServiceBDA != NULL)
	{
		DBG_LOG_APP_SYSTEM( "[APP_SYS] CheckIsAllowToTakeOver service BDA:0x%x%x, streaming data:%d %d %d",
			5, FW_bdaddr_to_2U32(pServiceBDA, TRUE), FW_bdaddr_to_2U32(pServiceBDA, FALSE),
			APP_AudioDspIsStreaming(pServiceBDA, AUDIO_A2DP), APP_AudioDspIsStreaming(pServiceBDA, AUDIO_SCO),
			APP_AudioDspIsStreaming(pServiceBDA, AUDIO_AMA));

		DBG_LOG_APP_SYSTEM( "[APP_SYS] CheckIsAllowToTakeOver streaming data2:%d %d ",
			2, APP_AudioDspIsStreaming(pServiceBDA, AUDIO_VAD), APP_AudioDspIsStreaming(pServiceBDA, AUDIO_XIAOAI));

		DBG_LOG_APP_SYSTEM( "[APP_SYS] CheckIsAllowToTakeOver,  AG num:%d, timer exist:%d, mcsync state:%d, OTA busy:%d",
			4, APP_GetAGNum(), APP_Conn_CheckTimer(TIMER_ID_ACL_LINK_TIMER), mcsyncState, g_is_fota_ongoing);
	}

	if(g_is_fota_ongoing)
	{
		return FALSE;
	}
...
=======
	 	...
	 	switch(APP_State_GetTopState(pServiceBDA))
	 	{
			...
			case APP_HFP_CAMULTY:
				isSCONotAllowTakeOverState = TRUE;
				break;
			default:
				break;
	 	}
		...
		if(APP_AudioDspIsStreaming(pServiceBDA, AUDIO_A2DP)
			|| (APP_AudioDspIsStreaming(pServiceBDA, AUDIO_SCO) && isSCONotAllowTakeOverState)
			|| APP_AudioDspIsStreaming(pServiceBDA, AUDIO_AMA))
		{
			return FALSE;
		}
