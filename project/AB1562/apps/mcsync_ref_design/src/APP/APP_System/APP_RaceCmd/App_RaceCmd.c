...
#include "app_racecmd_mobileapp.h"
...
U8 APP_RaceCmd_GetMsgNumInQueue(void)
{
	U8 totalMsgNum = 0, msgNum;

	if((msgNum = APP_RCmdKeyCode_GetMsgNumInQueue()))
	{
		totalMsgNum += msgNum;
		DBG_LOG_APP_SYSTEM( "[RHO_APP_RaceCmd] Old Agent KeyCode Not Allow:%d", 1, msgNum);
	}

	if((msgNum = APP_RCmdConn_GetMsgNumInQueue()))
	{
		totalMsgNum += msgNum;
		DBG_LOG_APP_SYSTEM( "[RHO_APP_RaceCmd] Old Agent Connection Not Allow:%d", 1, msgNum);
	}

	return (totalMsgNum + MSG_MessageSearchAllHandler((Handler)&gAppRaceCmdHandle));
}
