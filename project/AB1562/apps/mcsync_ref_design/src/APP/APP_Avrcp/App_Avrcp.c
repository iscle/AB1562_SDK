...
{	
	...
	U16 AttParaLen = msg->len;
	
	DBG_LOG_APP_AVRCP("[APP_AVRCP][TEST]AttParaLen:0x%x attNum:%d", 2,AttParaLen, msg->attNum);
	if(AttParaLen > 0)
	{
		if(msg->attNum == 1)
		{
			U32 attid = ENDIAN_TRANSFORM_U32(msg->getEleAttPara.singleAttrData.AttributeID);
			U16 attid_valuelen = ENDIAN_TRANSFORM_U16(msg->getEleAttPara.singleAttrData.AttributeValueLength);
			DBG_LOG_APP_AVRCP("[APP_AVRCP][TEST]attid:0x%x attid_valuelen:%d", 2,attid, attid_valuelen);
            //for(int i = 0;i<attid_valuelen;i++)
				//DBG_LOG_APP_AVRCP("[APP_AVRCP][TEST] attid_value[%d]:[0x%x]", 2,i, msg->getEleAttPara.singleAttrData.AttributeValue[i]);
		}		
	}
	else
	{
		DBG_LOG_APP_AVRCP("[APP_AVRCP][TEST]AttParaLen : 0", 0);
	}
