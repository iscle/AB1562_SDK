...
void APP_Qos_SetQos(BD_ADDR_T *pBdAddr, U8 serviceType)
{
	...
	if((BtAwsMce_IsSpecialLink(pBdAddr) && BtAwsMce_IsDefaultRoleAgent()) || BtAwsMce_GetDefaultRole() == ROLE_PARTNER)
		return;
...
U8 APP_Qos_GetMsgNumInQueue(void)
{
	return (MSG_MessageSearchAllHandler((Handler)&gAppQosHandler));
}
