...
void APP_VAD_On(void)
{
    AUDIO_DSP_START_PARA_STUR audioDspPara;
    audioDspPara.audioType = AUDIO_VAD;
    DSP_VAD_Bias_Enable(TRUE);
    APP_AudioDspPlay(&audioDspPara);
}

void APP_VAD_Off(void)
{
    BOOL is_stream = APP_AudioDspIsStreaming(NULL, AUDIO_VAD);
    AUDIO_DSP_STOP_PARA_STUR audioDspPara;
    audioDspPara.audioType = AUDIO_VAD;
    APP_AudioDspStop(&audioDspPara);
    if(is_stream)
    {
        DSP_VAD_Bias_Enable(FALSE);
    }
}
